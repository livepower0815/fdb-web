      ...
      <div class="chart" v-loading="chartLoading" element-loading-background="rgba(0, 0, 0, 0.5)">
        ...
        <div class="chart-main">
          <div class="chart-main-container">
            <ChartsCustomChart ref="linechart" :options="lineChartOptions" :chart-style="{ width: '100%', height: '100%' }" />
          </div>
        </div>
      </div>
      <!--線條圖 結束-->
...
export default {
  ...
  methods: {
    ...
    setChartData(mode) {
      switch (mode) {
        case 'all':
          this.lineChartOptions.series = this.seriesData.filter(() => true)
          break
        case 'person':
      ...
      this.$refs.linechart.$refs.CustomChart.refresh()
    }
  }
}
</script>
