<template>
  <div v-loading="isBindLoading" element-loading-background="rgba(0, 0, 0, 0.5)" class="personal-function-main exchange-site">
      ...
      <div class="main">
        <div>
          ...
          <div class="split-input" style="align-items: center; background-color: #252c3d; border-radius: 4px">
            <el-select v-model="formData.eid" class="fdb-select input select-M" popper-class="fdb-select" style="flex: 1; padding-left: 0">
              <el-option v-for="(csg, index) in csgList" :key="index" :label="csg.csgName" :value="csg.eid" />
            </el-select>
            <a v-if="formData.eid !== ''" class="text-link exchange-link" href="https://partner.bybit.com/b/fundobit" target="_blank">
    ...
    <div v-for="(bind, index) in bindList" :key="index" class="personal-function-exchange-block">
      ...
      <div>
        <img v-if="csgMap[bind.eid]" :src="csgMap[bind.eid].imageUrl" alt="bind-store-img" />
...
export default {
  ...
  computed: {
    ...
    selectedCsg() {
      const findData = this.csgList.find(item => item.eid === this.formData.eid)
      return findData || {}
    }
  },
  watch: {
  ...
  methods: {
    resetForm() {
      this.formData = {
        eid: this.csgList[0] ? this.csgList[0].eid : '',
        areaCode: '886',
        phone: '',
        fdB_UID: '',
        email: ''
