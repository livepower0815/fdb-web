        ...
        <table class="list-table" cellspacing="0" cellpadding="0" border="0">
          <thead>
            <tr>
              ...
              <th>{{ $t('member_name') }}</th>
              <th v-if="widthWithiIn(['MAX', 'XXL', 'XL', 'L', 'M'])" style="width: 85px; text-align: center">{{ $t('connect_info') }}</th>
          ...
          <tbody v-if="tableData.length > 0">
            <template v-for="(row, index) in tableData">
              <tr :key="`tr-1-${index}`">
                ...
                <td>
                  <span>{{ row.name }}</span>
                  <span v-if="widthWithiIn(['S'])" style="float: right; margin-left: 12px">
          ...
          <tbody v-else>
            <tr>
              <td style="padding: 10vw 0" colspan="12">
                ...
                <div style="color: #ddd">{{ $t('empty_data') }}</div>
              </td>
            </tr>
          </tbody>
        </table>
