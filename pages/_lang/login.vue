  ...
  <div class="login-bg">
      ...
      <div class="main">
        ...
        <div class="login-main">
          <div class="title flex-center" style="justify-content: flex-start">
            ...
            <div v-else v-loading="true" style="width: 80px; height: 35px" element-loading-background="rgba(0, 0, 0, 0.5)"></div>
        ...
        <a class="fdb-btn-primary-hover login-main-btn" @click.prevent="doLogin(false)">{{ $t('login') }}</a>

        <!-- google 註冊按鈕 -->
        <div class="flex-center" style="margin-top: 20px">
          <GoogleBtn :config="{ text: 'signin_with' }" @success="doLogin(true, $event)" />
        </div>
    ...
    <el-dialog :title="$t('login_fail')" :visible.sync="validateEmail.show" width="310px" :show-close="false" custom-class="fbd-dialog">
      <div v-if="!validateEmail.hasSent">
        <div style="margin-bottom: 12px; color: #eb4664; text-align: center">{{ $t('unconfirmed_account') }}</div>
...
export default {
  ...
  methods: {
    ...
    async doLogin(isGoogle = false, googleResponse = {}) {
      try {
        this.isLoading = true
        let res
        if (isGoogle) {
          ...
          // result = 1 的時候是登入成功
          res = await this.$api.user.googleLogin(postData)
        } else {
          // FDB 登入 驗證相關
          await this.validate()
          const postData = {
          ...
          // result = 1 的時候是登入成功
          res = await this.$store.dispatch('user/login', postData)
        }
