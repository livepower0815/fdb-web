  ...
  <div class="register-bg">
    <div v-loading="isLoading" element-loading-background="rgba(0, 0, 0, 0.5)" class="register-main-block">
      <div class="main">
        ...
        <div class="register-main">
          ...
          <div class="split-input">
            <el-select
              ...
              style="width: 90px; padding-left: 0; margin-right: 8px"
            >
              <el-option v-for="(phoneArea, index) in $map.phoneAreaCode" :key="index" :label="phoneArea.code" :value="phoneArea.code">
                <span style="float: left; margin-right: 26px; color: #ccc">{{ phoneArea.country }}</span>
        ...
        <div class="register-main">
          <div class="title">
            {{ $t('invite_code') }} <span style="color: #f3e411">( {{ $t('google_register') }} )</span>
          </div>
        ...
        <div class="register-main" style="display: flex">
          ...
          <div class="check-title">
            <i18n path="i_have_read_and_agree">
              <template #service>
                <NuxtLink :to="{ name: 'lang-disclaimer', params: { lang: $route.params.lang } }" v-slot="{ navigate }" custom>
                  <span class="text-link" @click="navigate">{{ $t('service') }}</span>
                </NuxtLink>
              </template>
              <template #privacy_policy>
                <NuxtLink :to="{ name: 'lang-privacy-policy', params: { lang: $route.params.lang } }" v-slot="{ navigate }" custom>
        ...
        點擊信箱url請導入登入畫面 -->
        <a class="register-main-btn fdb-btn-primary-hover" @click.prevent="doRegister(false)">{{ $t('register') }}</a>

        <!-- google 註冊按鈕 -->
        <div class="flex-center" style="margin-top: 20px">
          <GoogleBtn :config="{ text: 'signup_with' }" @success="doRegister(true, $event)" />
        </div>
...
export default {
  ...
  methods: {
    async doRegister(isGoogle = false, googleResponse = {}) {
      try {
        this.isLoading = true
        if (isGoogle) {
          // Google 註冊
          ...
          await this.$api.user.googleRegister(postData)
        } else {
          // FDB 註冊 驗證相關
          await this.validate()
          const postData = {
